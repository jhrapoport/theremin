...
def get_freq(note_name, steps_up=0):
    ...
    our_freq = const.BASE_FREQUENCY
    position = -1
    for i, general_note in enumerate(const.NOTES):
        position = i
        if our_letter == general_note:
            break
        # if that wasn't it, go up a note here
        our_freq *= 2**(1/12)
    difference = int(note_name[-1]) - int(const.BASE_NOTE[-1])
    # octave changes between b and c, so adjust for that
    if position >= const.OCTAVE_DIVIDER:
        difference -= 1
    if difference == 0:
        return our_freq
